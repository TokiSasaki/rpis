...
static void changeOwner(char *cmd, char *file)
{
    ...
    if (chown(file, uid, gid) != 0) {
        ...
        // Removed (ignoring) the check for not existing as I'm fed-up with
        // morons telling me that
...
static void checkDevTree(char *argv[])
{
    ...
    if (stat("/proc/device-tree", &statBuf) ==
        0) // We're on a devtree system ...
    {
        fprintf(stderr, "%s: Unable to load/unload modules as this Pi has the "
                        "device tree enabled.\n"
                        "  You need to run the raspi-config program (as root) "
                        "and select the\n"
                        "  modules (SPI or I2C) that you wish to load/unload "
                        "there and reboot.\n"
...
static void doLoad(int argc, char *argv[])
{
    ...
    /**/ if (strcasecmp(argv[2], "spi") == 0) {
        ...
        file2 = "/dev/spidev0.1";
        if (argc == 4) {
            fprintf(stderr, "%s: Unable to set the buffer size now. Load "
                            "aborted. Please see the man page.\n",
                    argv[0]);
            exit(1);
        } else if (argc > 4)
            _doLoadUsage(argv);
...
void doExport(int argc, char *argv[])
{
    ...
    /**/ if ((strcasecmp(mode, "in") == 0) || (strcasecmp(mode, "input") == 0))
        ...
        fprintf(stderr,
                "%s: Invalid mode: %s. Should be in, out, high or low\n",
                argv[1], mode);
        exit(1);
    }
...
void doWfi(int argc, char *argv[])
{
    ...
    /**/ if (strcasecmp(argv[3], "rising") == 0)
        ...
        fprintf(
            stderr,
            "%s: wfi: Invalid mode: %s. Should be rising, falling or both\n",
            argv[1], argv[3]);
        exit(1);
    }
...
void doEdge(int argc, char *argv[])
{
    ...
    if ((fd = fopen(fName, "w")) == NULL) {
        fprintf(stderr,
                "%s: Unable to open GPIO edge interface for pin %d: %s\n",
                argv[0], pin, strerror(errno));
        exit(1);
    }
    ...
    // Change ownership of the value and edge files, so the current user can
    // actually use it!
...
{
    printf("GPIO Reset is dangerous and has been removed from the gpio "
           "command.\n");
    printf(" - Please write a shell-script to reset the GPIO pins into the "
           "state\n");
...
static void doUsbP(int argc, char *argv[])
{
    ...
    if (!((model == PI_MODEL_BP) || (model == PI_MODEL_2))) {
        fprintf(stderr,
                "USB power contol is applicable to B+ and v2 boards only.\n");
        exit(1);
    }
    ...
    if ((strcasecmp(argv[2], "high") == 0) ||
        (strcasecmp(argv[2], "hi") == 0)) {
...
static void doGbw(int argc, char *argv[])
{
    ...
    if (gertboardAnalogSetup(64) < 0) {
        fprintf(stderr,
                "Unable to initialise the Gertboard SPI interface: %s\n",
                strerror(errno));
        exit(1);
    }
...
static void doPwmMode(int mode) { pwmSetMode(mode); }
...
static void doVersion(char *argv[])
{
    ...
    printf("  Type: %s, Revision: %s, Memory: %dMB, Maker: %s %s\n",
           piModelNames[model], piRevisionNames[rev], piMemorySize[mem],
           piMakerNames[maker], warranty ? "[Out of Warranty]" : "");
    ...
    if (stat("/proc/device-tree/model", &statBuf) ==
        0) // Output Kernel idea of board type
    {
...
int main(int argc, char *argv[])
{
    ...
    if (strcasecmp(argv[1], "-warranty") == 0) {
        ...
        printf("    This program is free software; you can redistribute it "
               "and/or modify\n");
        printf("    it under the terms of the GNU Leser General Public License "
               "as published\n");
        printf("    by the Free Software Foundation, either version 3 of the "
               "License, or\n");
        ...
        printf("\n");
        printf("    You should have received a copy of the GNU Lesser General "
               "Public License\n");
    ...
    if (geteuid() != 0) {
        fprintf(stderr, "%s: Must be root to run. Program should be suid root. "
                        "This is an error.\n",
                argv[0]);
        return 1;
    }
